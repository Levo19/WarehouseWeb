...
class App {
    ...
    renderZonePickup(zone, container) {
        ...
        // 1. FILTERING LOGIC: Cumulative from 29/12/2025
        ...
        const isOnOrAfterBaseline = (dateStr) => {
            ...
            if (!d) return false;
            // Compare timestamps (ignoring time)
        ...
        this.data.requests.forEach(req => {
            ...
            if (cat === 'solicitado') {
                ...
                // Add to FIFO Queue
                aggregator[codeKey].pendingQueue.push({ id: req.idSolicitud, qty: qty, ts: reqTs });
        ...
        Object.values(aggregator).forEach(item => {
            ...
            // Sort pending queue by Date (Oldest First - FIFO)
            item.pendingQueue.sort((a, b) => a.ts - b.ts);
            ...
            // Strict check for floating point errors
            else if (Math.round(pendingQty * 100) / 100 > 0) {
                // Pass CODE as useId for pending items to enable Batch Separation
                pendingList.push({ ...item, qtyToShow: pendingQty, type: 'pending', useId: item.code });
            }
        });
        ...
        const renderCard = (item, isPending) => {
            ...
            if (isPending && item.allRequests && item.allRequests.length > 0) {
                requestListHtml = item.allRequests.map(r => {
                    let shadowColor = '#ccc';
                    let borderColor = '#ccc';
                    if (r.status === 'solicitado') { shadowColor = 'rgba(46, 204, 113, 0.6)'; borderColor = '#2ecc71'; }
                    else if (r.status === 'separado') { shadowColor = 'rgba(243, 156, 18, 0.6)'; borderColor = '#f39c12'; }
            ...
            // Button Action Logic
            // If Pending: Call moveToSeparated with CODE (item.useId is code for pending)
            // If Separated: item.useId is the last ID (used for delete/edit)
            ...
            return `
                    <div class="product-card request-card" data-search="${searchTerms}" onclick="this.classList.toggle('flipped')" style="${cardOpacity}">
                        <div class="product-card-inner" style="border-left: 4px solid ${isPending ? (isOutOfStock ? '#ef4444' : 'var(--primary-color)') : '#2e7d32'};">
                            <!-- FRONT -->
                            <div class="card-front">
                                <div class="card-img-container" style="height:140px;">${imgHtml}</div>
                                <div class="card-content">
    ...
    async moveToSeparated(btnElement, codeOrId) {
                        // Param is now 'codeOrId'. For Pending, it is CODE. For Separated, it is ID.
                        // We act on PENDING items here.

                        const qtyInput = document.getElementById(`qty-${codeOrId}`);
                        if (!qtyInput) {
                            console.error('Input not found for:', codeOrId);
                            return;
                        }
                        ...
                        // We need to re-aggregate or find the item.
                        // EASIEST: The 'pendingList' creation logic stores everything. 
                        // But we don't have access to 'pendingList' here cleanly without re-running logic.
                        // Let's re-find the product requests to be safe and authoritative.

                        // 1. RE-AGGREGATE single product to get Queue
                        ...
                        const productRequests = this.data.requests.filter(r =>
                            ...
                            (String(r.idSolicitud).startsWith('temp-') || (parseDate(r.fecha) && parseDate(r.fecha).getTime() >= BASELINE_DATE.getTime()))
                        );
                        ...
                        productRequests.forEach(r => {
                            ...
                            if (cat === 'solicitado') {
                                if (!requestMap[r.idSolicitud]) requestMap[r.idSolicitud] = { id: r.idSolicitud, initial: 0, separated: 0, ts: 0 };
                                ...
                                // Separated rows might not link to specific ID in some legacy cases, but assume they do or we grouped them.
                                // In our system 'separateRequest' links via ID.
                                // Wait, we need to know WHICH request ID this separation belongs to.
                                // The 'separateRequest' payload has 'idSolicitud'.
                                // The 'solicitudes' sheet doesn't store 'parent ID' in a dedicated column, 
                                // IT USES COLUMN A (ID).
                                // When we append a row for 'SEPARADO', do we use the SAME ID?
                                // YES. separateRequest(payload) -> appends row with `idSolicitud` (the same ID).
                                // So multiple rows share the same ID.
                        ...
                        if (newQty > totalAvailable + 0.001) {
                            alert(`Cantidad excede lo pendiente (${totalAvailable.toFixed(2)})`);
                            return;
                        }
                        ...
                        fetch(API_URL, {
                            ...
                            .then(result => {
                                if (result.status === 'success') {
                                    console.log('Batch separation success', result);
                                    // We don't need to swap IDs specifically because this was a batch.
                                    // The server created multiple rows.
                                    // On next refresh, we will get the real rows.
                                    // The Temp item is fine to stay until refresh.
                                } else {
                                    throw new Error(result.message || 'Error en batch');
                                }
                            })
                            .catch(err => {
